...
$(document).ready(function() {
  ...
  // Populate select menu Origin
  var buildSelect = function(lines) {
    $.each(lines, function(line, stops) {
      // Create optgroup using jQuery 
      var optgroup = $('<optgroup/>');
      // Add label to line key
      optgroup.addClass(line).attr( 'label', line + ' Line' );
    // Populate select menu Destination
      $.each(stops, function(index, name) {
        // Create options for optgroup using jQuery 
        var option = $('<option/>');
        option.addClass(name).html(name);
        optgroup.append(option);
      });
      // Append the jQuery optgroup and options
      $('#origin, #destination').append(optgroup);
    });
  }

  // Call populate menu function on the subway hash
  buildSelect(subway);
